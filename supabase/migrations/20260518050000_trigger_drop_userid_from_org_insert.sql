-- ─────────────────────────────────────────────────────────────────────────────
-- Remove user_id from the organizations INSERT (column being dropped).
-- Also removes SET search_path = '' — all references are fully schema-qualified
-- so the restriction is unnecessary and causes issues in the dashboard editor.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF v_company IS NOT NULL AND v_company <> '' THEN
    ...
    INSERT INTO public.organizations (org_name)
    VALUES (v_company)
  ...
  -- Write org_sk and user_profile back so the client can read them via getUser()
  UPDATE auth.users
